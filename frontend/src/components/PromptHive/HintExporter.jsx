...
import { generateRandomString, getCurrentTimestamp } from "../../utilities/utils";
import {
...
const HintExporter = ({
  ...
  const [outputSpreadsheetId, setOutputSpreadsheetId] = useState("1Jbrh3WBHgiuWkuv1aXa2NMKkMVK8CpDhMjep1R8qNeA");
  ...
  useEffect(() => {
    if (allHintsAndScaffolds.length > 0) {
      console.log("Hints and scaffolds are ready for export:", allHintsAndScaffolds);
    }
  ...
  async function clearSheetContents(spreadsheetId, sheetName) {
    ...
    console.log(`Cleared contents in range '${clearRange}' of sheet '${sheetName}'`);
  }
      ...
      for (const sheetName in sheetData) {
        if (sheetData.hasOwnProperty(sheetName)) {
          ...
          if (headers.length > 0) {
            ...
            console.log(`Headers appended successfully to sheet '${sheetName}'.`);
          }
  ...
  const exportToSheet = async () => {
    ...
    selectedSheetData = selectedSheetData.map(subarray => subarray.slice(0, 16));
    ...
    colHeaders = colHeaders.map(header => lessonContentAttributes[header] || header);
  ...
  return (
    <>
      <div>
        <label htmlFor="outputSheetURL">Output Sheet URL:</label>
        <input
          type="text"
          id="outputSheetURL"
      ...
      <div>
        <label htmlFor="outputSheetName">Output Sheet Name:</label>
        <input
          type="text"
          id="outputSheetName"
      ...
      <Button onClick={exportToSheet} disabled={exporting}>
